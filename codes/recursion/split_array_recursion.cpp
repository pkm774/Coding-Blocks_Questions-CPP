...
void split_array(std::vector<int>& arr, int index, int& count, int sum, std::vector<int> checkElement) {
	...
	if (index == arr.size()) {
		...
		int included_sum = 0;
		// Calculate sum of included elements
...
int main() {
	...
	// Create an integer vector and fill it with zeroes.
	std::vector<int> checkElement(size, 0);
